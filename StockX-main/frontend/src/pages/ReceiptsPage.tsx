...
import { receiptsApi, inventoryApi } from '../services/api';
  ...
  const startPolling = () => {
    ...
    pollingRef.current = setInterval(async () => {
      ...
      const stillProcessing = data.some(
        (r: Receipt) => r.status === 'PROCESSING',
      );

      if (!stillProcessing) {
        stopPolling();

        // 🔥 REFRESH INVENTAR DUPĂ COMPLETED
        ...
        {uploading ? (
          ...
          <div className="flex flex-col items-center">
            ...
            <p className="text-gray-600">
              {isDragActive
                ? 'Lasă bonul aici'
                : 'Trage și lasă un bon aici sau fă clic pentru a selecta'}
            </p>
            <p className="text-sm text-gray-500 mt-2">
              Fișiere PDF sau imagini
            </p>
          </div>
        )}
      </div>
            ...
            <li key={receipt.id}>
              <div className="px-4 py-4 sm:px-6 flex items-center justify-between">
                <div className="flex items-center space-x-4">
                  ...
                  <div>
                    ...
                    <div className="mt-1 text-sm text-gray-500">
                      ...
                      {receipt.totalAmount && (
                        <span className="ml-4">
                          Total: {receipt.totalAmount.toFixed(2)} RON
                        </span>
                      )}
                      <span className="ml-4">
                        Status:{' '}
                        <span
                          className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${receipt.status === 'COMPLETED'
                ...
                <div className="flex items-center space-x-2">
                  <a
                    href={
                      receipt.fileUrl.startsWith('http')
                        ? receipt.fileUrl
                        : `http://localhost:3000${receipt.fileUrl}`
                    }
                    target="_blank"
